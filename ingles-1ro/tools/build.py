# -*- coding: utf-8 -*-
"""
Builds the interactive HTML version of the 1ro de Secundaria Ingles Libro Abierto.
Renders every PDF page as a JPEG background image, extracts text blocks with their
bounding boxes, detects which blocks are English (vs the book's Spanish rubric/
footer text), pre-generates a real neural-voice MP3 per unique speakable text via
edge-tts, and writes data/pages.js for the HTML/JS reader to consume.

Audio is pre-baked (not read live via the browser's speechSynthesis) so playback
is identical and fully offline-reliable regardless of what voices are installed on
the viewing machine — the only step that needs internet is running this script.

Rerun this whenever the source PDF changes. Safe to rerun any time: pages/ and
data/ are fully regenerated, and audio/ is regenerated incrementally (existing
clips whose text hasn't changed are left alone, so a rerun after a small edit is
fast). index.html/css/js are hand-authored and never touched by this script.
"""
import asyncio
import fitz
import hashlib
...
SRC_PDF = os.path.join(os.path.dirname(ROOT), "Lenguas Extranjeras - Inglés 1° Secundaria (Libro Abierto).pdf")
...
def clean_for_speech(text):
    # Mirrors js/reader.js's cleanForSpeech() exactly — strips leading list markers
    # ("1.", "a)", "F.", etc.) at the start of the text and after inner whitespace,
    # so a numbered/lettered zone speaks only its content, not the marker. Kept in
    # sync by hand; if one changes, change the other.
...
def is_unit_opener(page_text):
    # "Unit N" alone appears on an early legend page too (false positive); "Unit Content"
    # only appears on the real two-page unit-opener spread, once per unit.
    return "Unit Content" in page_text
...
def main():
    ...
    for i, page in enumerate(doc):
        ...
        # The "How your book works" explainer page (~page 6) embeds a miniature
        # reproduction of Unit 1's opener page, text layer and all, which would
        # otherwise register as a false extra unit. Real unit openers never start
        # this early, so a hard floor rejects the explainer page specifically.
        if is_unit_opener(page_text) and page_no >= 10 and (not units or units[-1]["page"] != page_no - 1):
    ...
    # Emitted as a plain <script> global (not fetch()-ed JSON) so index.html works by
    # simple double-click. fetch() of local files is blocked by Chrome/Edge when opened
    # via file:// (no server); a <script src> tag has no such restriction.
